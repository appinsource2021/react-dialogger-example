...
import {
    ActionDialogDef,
    ActionDef, IDialogApiDef, IDialogSize
...
const DialogBody = ({dialog}: {dialog: IDialogApiDef}) => {
    ...
    return (
        <div>
            <span>Orders: {dialog.values.cachedOrders?.length}</span>
        </div>
    );
}
...
const useUpdateValues = () => {
    ...
    // const { data: cachedOrders, isLoading } = useQuery({
    //     queryKey: ['cachedOrders'],
    //     queryFn: () => fetchData(),
    //     // initialData: dialog.values.orders,
    //     // staleTime: Infinity, // süresiz cache
    // });
    ...
    const openDialog = (apiRef: React.RefObject<IDialogApiDef>) => {
        ...
        setTimeout(() => {
            apiRef.current?.setInProcess(true);
            setTimeout(()=>{
                apiRef.current?.close();
            }, 2000)
            setTimeout(()=>{
                apiRef.current?.setInProcess(false);
            }, 4000)
        ...
        okAction.onClick((button, dialog ) => {
        ...
        dialog
            .setHeader( (dialog: IDialogApiDef ) => {
                // dialog.onResize( (size, dialog1) => {
                //     console.log('Dialog_Resize_by_Header', size );
                // })
                return <div>
            ...
            .setBody( (dialog: IDialogApiDef) => {
                ...
                apiRef.current?.processingListener( process => {

                    console.log('processingListener_outside', process);
            ...
            .resizeListener( (size: IDialogSize, dialog: IDialogApiDef) => {

                console.log('Dialog_Resize_by_Init', size );
            })
