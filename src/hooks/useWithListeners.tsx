...
const DialogBody = ({dialog}: {dialog: IDialogApiDef}) => {

    const listenerHook = useWithListeners();


    // dialog.processingListener( inProcess => {
    //     console.log('DialogBodyListener', inProcess)
    // })

    // console.log('DialogBodyListener_IsInProcess', dialog.isInProcess());
    ...
    return <div>
        ...
        <table>
            <thead>
            <tr>
                <th align={'left'}>Idalog in Process: {dialog.isInProcess().inProcess ? 'True' : 'False'}</th>
            ...
            <tr>
                ...
                <td align={'left'}><input onChange={ event =>{
                    dialog.setValue('name', event.target.value);
                }} /></td>
                <td>
                    {dialog.values.date}
                </td>
...
const useWithListeners = () => {
    ...
    const openDialog = (id?: any) => {
        ...
        countAction.onClick( (button, dialog1) => {

            console.log('OkClick_values', dialog1);
            const newValue = dialog1?.values!.count + 1;
            dialog1?.setValue?.('count', newValue)
            button.updateOptions({
                label: `Count: ${newValue}`,
            });
            openDialog( (dialog1?.dialogOptions.base?.id ?? 0) as number + 1);
        });
        ...
        dialog
            .setHeader( (dialog: IDialogApiDef) => <div>
                <span>Super-dialog-22 {dialog.isInProcess().message} </span>
                <span>Super-dialog-22 {dialog.dialogOptions.base?.id} </span>
                <span>{dialog.values.name} {dialog.values.surname}</span>
            </div> )
            // .setHeader('Header')
            .setBody( (dialog: IDialogApiDef) => {
                console.log('setBody_dialog', dialog.values);
                return <DialogBody dialog={dialog} />
            } )
            .addActions([
                cancelAction,
                countAction,
            ...
            .keyboardListener((key, dialog) => {
                console.log('keyboardListener', key);
            }).resizeListener((size, dialog) => {
                console.log('resizeListener', size);
            })
            .stateListener((values, dialog) => {
